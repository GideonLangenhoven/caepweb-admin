    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-5 lg:col-span-2">
          ...
          <div className="mt-4 flex flex-wrap items-end gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4">
            <label className="text-sm text-gray-700">
              Switch plan
              <select value={targetPlan} onChange={(e) => setTargetPlan(e.target.value)} className="mt-1 block rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm">
                {plans.map((p) => (
                  <option key={p.id} value={p.id}>{p.name} ({formatZar(p.monthly_price_zar)}/mo)</option>
                ))}
              </select>
            </label>
            <button onClick={switchPlan} disabled={working || !targetPlan} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 disabled:opacity-50">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-5 lg:col-span-2">
          ...
          <button onClick={createLandingPageOrder} disabled={working} className="mt-4 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">
